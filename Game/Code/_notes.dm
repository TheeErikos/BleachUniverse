 ...
 * Set the anchor property of the chat input focus so the window resizes
   properly now.
 ...
 * Changed how chat messages are colored. Instead of using color constants from
   the global Constants object, it now uses client/script to focus the color.
 ...
 * Added the ReiCombat object to the sample game which has slightly different
   rules than the PhysicalCombat object. It uses the focus stat for critical hits
   and the resistance stat for damage reduction.
 * Added the focus and resistance stats which are used by the new ReiCombat
   object.
 ...
 * Expanded the sample game's stats to include effectiveattack, effectivespeed, and effectivedefense instead
